...
async function escogerPregunta() {
  ...
  if (contadorPreguntas == _baseDeDatos.cantidadPreguntas) {
    //Con esta condición se determina que el juego ha finalizado porque la cantidad de preguntas resueltas son todas las que hay en la base de datos
    await swal.fire(
      "El simulador ha finalizado",
      "Sus aciertos son: " + puntaje + "/" + _baseDeDatos.cantidadPreguntas,
      "success"
...
async function respondeIncorrectamente() {
  await swal.fire(
    ...
    "La respuesta es: " + preguntaSeleccionada.respuesta
    + ". Visitanos en el Pre para aprender como resolverlo.",
    "error"
  );
...
async function terminarJuego() {
  ...
  if (nombre) {
    _puntaje_historial.adicionarAlHistorial(
      nombre,
      puntaje + "/" + _baseDeDatos.cantidadPreguntas
    );
  }
  _puntaje_historial.actualizarTablaHistorial();
  _controlHTML.cambiarPantalla("presentacion");
  _puntaje_historial.borrarPuntaje();
}
